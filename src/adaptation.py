    ...
    t1 = torch.cat(
        (
            torch.cat(
                tuple(
                    map(
                        lambda gp: get_gaussian_points_physical_space(gp[0], gp[1], 7).reshape(-1),
                        zip(x[:-1], x[1:]),
                    )
                )
            ),
...
def refine(base_x: torch.Tensor, loss_fun: Callable, num_max_points: int, tol: float):
    ...
    x = base_x.cpu().detach().clone().requires_grad_(True)
    n_points = x.numel()
    refined = True
    ...
    while (n_points < num_max_points and refined):
        ...
        for x1, x2 in zip(x[:-1], x[1:]):
            int_x = torch.linspace(x1.item(), x2.item(), 20).requires_grad_(True).reshape(-1, 1)
        ...
        x = torch.cat((x, torch.tensor(new_points))).sort()[0]
        n_points = x.numel()
